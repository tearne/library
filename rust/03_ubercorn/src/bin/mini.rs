...
impl UnicornMini {
    pub fn new() -> Self {
        ...
        let mut um = UnicornMini {
            ...
            buttons: [
                gpio.get(BUTTON_A).unwrap().into_input(),
                gpio.get(BUTTON_B).unwrap().into_input(),
                gpio.get(BUTTON_X).unwrap().into_input(),
                gpio.get(BUTTON_Y).unwrap().into_input()
            ]
        };
    ...
    fn write_prefix(&mut self, prefix: &[u8], data: &[u8]){
        ...
        for i in 0..2 {
            let pin = &mut self.cs_pins[i];
            pin.set_low(); //Chip select
            self.spi.write(prefix).expect("SPI write error");
            if data.len() > 0 {
                let d = &data[Self::buf_offset(i)];
                self.spi.write(d).expect("SPI write error");
                // println!("Buffer {}, sent {}:\n{:?}", i, d.len()/3, d);
            }
            pin.set_high();
        }
