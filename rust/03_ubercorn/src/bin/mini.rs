...
async fn main() {
    ...
    //https://docs.rs/gpio-cdev/0.4.0/gpio_cdev/struct.Line.html#method.events
    ...
    // fn button_receiver(button: &Line, consumer_name: Cow<'static, str>) -> Receiver<()> {
    ...
    //         for _ in events {
    //             sender.try_send(()).expect(&format!("panic on {}", consumer_name));
    //         }
    //     });
    ...
    // let mut a = button_receiver(&um.buttons[0], "button_a_listener".into());
    // let mut b = button_receiver(&um.buttons[1], "button_b_listener".into());
    // let mut x = button_receiver(&um.buttons[2], "button_x_listener".into());
    // let mut y = button_receiver(&um.buttons[3], "button_y_listener".into());
    ...
    let (b_A_sender, mut b_A_receiver) = channel(16);
    let (b_B_sender, mut b_B_receiver) = channel(16);
    let (b_X_sender, mut b_X_receiver) = channel(16);
    let (b_Y_sender, mut b_Y_receiver) = channel(16);

    let events_A = um.buttons[0].events(
        LineRequestFlags::INPUT, 
        EventRequestFlags::BOTH_EDGES,
        "um-but-a")
        .unwrap();
    let b_A_blocking_task = tokio::task::spawn_blocking(move || {
        for _ in events_A {
            println!("a");
            b_A_sender.try_send(()).unwrap();
        }
    });

    let events_B = um.buttons[1].events(
        LineRequestFlags::INPUT, 
        EventRequestFlags::BOTH_EDGES,
        "um-but-b")
        .unwrap();
    let b_B_blocking_task = tokio::task::spawn_blocking(move || {
        for _ in events_B {
            println!("b");
            b_B_sender.try_send(()).unwrap();
        }
    });

    let events_X = um.buttons[2].events(
        LineRequestFlags::INPUT, 
        EventRequestFlags::BOTH_EDGES,
        "um-but-x")
        .unwrap();
    let b_X_blocking_task = tokio::task::spawn_blocking(move || {
        for _ in events_X {
            println!("x"); 
            b_X_sender.try_send(()).unwrap();
        }
    });

    let events_Y = um.buttons[3].events(
        LineRequestFlags::INPUT, 
        EventRequestFlags::BOTH_EDGES,
        "um-but-y")
        .unwrap();
    let b_Y_blocking_task = tokio::task::spawn_blocking(move || {
        for _ in events_Y {
            println!("y");
            b_Y_sender.try_send(()).unwrap();
        }
    });
    ...
    let (_,_,_,_,_) = tokio::join!(
        ...
        tokio::task::spawn(async move {
            loop {
                tokio::select! {
                    Some(_) = b_A_receiver.recv() => {
                        println!("Button A")
                    }
                    Some(_) = b_B_receiver.recv() => {
                        println!("Button B");
                    }
                    Some(_) = b_X_receiver.recv() => {
                        println!("Button X")
                    }
                    Some(_) = b_Y_receiver.recv() => {
                        println!("Button Y");
                    }
                    else => break
                };
