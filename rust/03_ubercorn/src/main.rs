...
use unicorn::{display::pixel::*, display::*, error::Error, filesystem::*, keyboard::*};
...
impl Dependencies {
    pub fn init() -> Self {
        ...
        art.insert(
            Mode::ZOMBIE,
            Graphic::load(Path::new("resources/zombie.json")).as_pixels(),
        );
        art.insert(
            Mode::SHEEP,
            Graphic::load(Path::new("resources/sheep.json")).as_pixels(),
        );
...
fn do_twinkle(
    ...
    loop {
        ...
        let rendered: Vec<RGB> = pixels.iter_mut().map(|px| px.evolve_and_get()).collect();
